...
const FoundUpload = (props) => {
  ...
  const foundRef=collection(db,'FoundUpload')
  ...
  const handleFormSubmit = async (e) => {
    ...
    // const storageRef= ref(imageDb,`files/${image}`)
    ...
    try {
      ...
      await uploadImageToFirestore('images/'+auth.currentUser.uid, image).then(url => {
        console.log(url)
        addDoc(foundRef,{
          ...
          console.log("objec written in this id",res.id)
          ...
          <label style={{color: `${props.theme === 'dark' ? '#f5f5f5' : ''}`}} htmlFor="itemImage" type="file">Image
            <div>
              <input className='select pt-1'
                ...
                style={{backgroundColor: `${props.theme === 'dark' ? 'rgb(74 72 72)' : ''}`, color: `${props.theme === 'dark' ? '#f5f5f5' : ''}`,}}
                onChange={(e) => {
                  // console.log('e.target.files[0])', e.target.files[0],auth.currentUser.uid)
                  // console.log(auth.currentUser.uid)
                  setImage(e.target.files[0])}
                  // uploadImageToFirestore('images/'+)
                }
                required
              />
            </div>
